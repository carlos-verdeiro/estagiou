$(document).ready(function () {
    ...
    const feedbackNomeEmpresa = $("#feedback-nomeEmpresa");
    ...
    //etapa 3
    const atuacao = $("#atuacao");
    const feedbackAtuacao = $("#feedback-atuacao");
    ...
    nomeEmpresa.on("blur", function () {
        validacaoTamanho(nomeEmpresa, feedbackNomeEmpresa, 'minimo', 0);
    });
    ...
    atuacao.on("blur", function () {

        validacaoTamanho(atuacao, feedbackAtuacao, 'minimo', 0);
    ...
    $('#formEtapa1').submit(async function (event) {
        ...
        try {
            ...
            if (cnpjF && validacaoTamanho(nomeEmpresa, feedbackNomeEmpresa, 'minimo', 0) && validacaoTamanho(telefone, feedbackTelefone, 'igual', 14) && emailF) {
    ...
    $('#formEtapa4').submit(function (event) {
        ...
        // Realiza a validação do campo antes do envio
        if (validacaoTamanho(atuacao, feedbackAtuacao, 'minimo', 0) && validacaoTamanho(descricao, feedbackDescricao, 'minimo', 0)) {
            // Se a validação for bem-sucedida, prossegue com o envio do formulário
            this.submit();
        } else {
            // Se a validação falhar, exibe mensagem ou realiza ação necessária
